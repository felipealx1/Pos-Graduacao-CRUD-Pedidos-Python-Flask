...
def lista_clientes():
    try:
        ...
        for cliente in clientes:
            ...
            clientes_serializdo.append(cliente)

        return jsonify(clientes_serializdo), 200
...
def set_cliente():
    ...
    try:
        ...
        if resultado.inserted_id:
            ...
            return "Erro ao inserir o cliente", 500
    except Exception as e:
        print(f"Error: {e}")
        return "Erro ao inserir o cliente", 500
...
@app.route("/excluirCliente/<id_cliente>", methods=['DELETE'])
...
def set_produto():
    ...
    try:
        ...
        if resultado.inserted_id:
            ...
            return "Erro ao inserir o produto", 500
    except Exception as e:
        print(f"Error: {e}")
        return "Erro ao inserir o produto", 500
...
@app.route("/excluirProduto/<id_produto>", methods=['DELETE'])
...
def set_pedido():
    ...
    # Verificar se o produtu existe
    produto = produtos_collection.find_one({"id_produto": id_produto})
    if not produto:
        return jsonify({"error": "Produto não encontrado"}), 404

    # Gerar um ID de pedido único 
    id_pedido = str(ObjectId()) 

    # Definir o valor do pedido 
    valor = produto.get('preco', 0)  
...
def lista_pedidos():
    try:
        ...
        for pedido in pedidos:
            ...
            pedidos_serializados.append(pedido)
...
def update_pedido(id_pedido):
    try:
        dados = request.get_json()
        pedido = pedidos_collection.find_one({"_id": ObjectId(id_pedido)})
...
def delete_pedido(id_pedido):
    try:
        ...
        if resultado.deleted_count == 1:
            return f"Pedido {id_pedido} excluído com sucesso.", 200
        else:
            return jsonify({"error": "Erro ao excluir pedido"}), 500
